// APIntTest.cpp - APInt unit tests
//
// Part of the bijou Project, under the Apache License v2.0 with LLVM Exceptions.
//
// This file is a modified copy of the file "unittest/ADT/APIntTest.cpp" from the LLVM Project.
...
TEST(APIntTest, FromArray) {
  uint64_t arr[1] = {1};
  EXPECT_EQ(APInt(32, uint64_t(1)), APInt(32, std::span<uint64_t>(arr)));
}
...
TEST(APIntTest, StringDeath) {
  ...
  EXPECT_DEATH((void)APInt(32, std::string_view("1\02", 3), 10), "Invalid character in digit string");
...
TEST(APIntTest, GetMostSignificantDifferentBitExaustive) {
  auto GetHighestDifferentBitBruteforce =
    ...
    bijou_unreachable("Must have found bit mismatch.");
  };
...
TEST(APIntTest, ZeroWidth) {
  ...
  uint64_t arr[3] = {0, 1, 2};
  EXPECT_EQ(0U, APInt(0, std::span<uint64_t>(arr)).getBitWidth());

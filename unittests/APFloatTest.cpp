// APFloatTest.cpp -- APFloat unit tests
//
// Part of the bijou Project, under the Apache License v2.0 with LLVM Exceptions.
//
// This file is a modified copy of the file "unittests/ADT/APFloatTest.cpp" from the LLVM Project.
...
//   * Replaced uses of LLVM's modified gtest with Google's original gtest.
//
...
#include "bijou_unittest_helpers.hpp"
...
static std::string convertToErrorFromString(std::string_view Str) {
  bijou::APFloat F(0.0);
  auto StatusOrErr =
      F.convertFromString(Str, bijou::APFloat::rmNearestTiesToEven);
  EXPECT_TRUE(!StatusOrErr);
  return std::string(StatusOrErr.takeError().take().value());
}

static double convertToDoubleFromString(std::string_view Str) {
  bijou::APFloat F(0.0);
  auto StatusOrErr =
      F.convertFromString(Str, bijou::APFloat::rmNearestTiesToEven);
...
TEST(APFloatTest, DecimalStringsWithoutNullTerminators) {
  ...
  // rdar://14323230.
  EXPECT_EQ(convertToDoubleFromString(std::string_view("0.00", 3)), 0.0);
  EXPECT_EQ(convertToDoubleFromString(std::string_view("0.01", 3)), 0.0);
  EXPECT_EQ(convertToDoubleFromString(std::string_view("0.09", 3)), 0.0);
  EXPECT_EQ(convertToDoubleFromString(std::string_view("0.095", 4)), 0.09);
  EXPECT_EQ(convertToDoubleFromString(std::string_view("0.00e+3", 7)), 0.00);
  EXPECT_EQ(convertToDoubleFromString(std::string_view("0e+3", 4)), 0.00);
}
...
TEST(APFloatTest, fromZeroDecimalLargeExponentString) {
  ...
  EXPECT_EQ(0.0,  APFloat(APFloat::IEEEdouble(), std::string_view("0e1234" "\0" "2", 6)).convertToDouble());
}
...
TEST(APFloatTest, fromStringSpecials) {
  ...
  for (size_t I = 0; I < array_lengthof(NaNPayloads); ++I)
    NaNPayloadDecStrings[I] = std::to_string(NaNPayloads[I]);
  ...
  const std::string_view NaNStrings[] = {"nan", "NaN"};
  for (std::string_view NaNStr : NaNStrings)
      ...
      for (size_t J = 0; J < array_lengthof(NaNPayloads); ++J) {
        ...
        for (char SignChar : Signs) {
          ...
          for (size_t K = 0; K < NumTestStrings; ++K) {
            std::string_view TestStr = TestStrings[K];

            APFloat F(Sem);
            bool HasError = !F.convertFromString(
                TestStr, bijou::APFloat::rmNearestTiesToEven);
  ...
  const std::string_view InfStrings[] = {"inf",  "INFINITY",  "+Inf",
                                  "-inf", "-INFINITY", "-Inf"};
  for (std::string_view InfStr : InfStrings) {
    bool Negative = InfStr.front() == '-';

    APFloat F(Sem);
    bool HasError =
        !F.convertFromString(InfStr, bijou::APFloat::rmNearestTiesToEven);
    EXPECT_FALSE(HasError);
...
TEST(APFloatTest, toString) {
  ...
  {
    std::string Str;
    const uint64_t arr[] = {0, 1};
    APFloat UnnormalZero(APFloat::x87DoubleExtended(), APInt(80, arr));
...
TEST(APFloatTest, StringDecimalError) {
  ...
  EXPECT_EQ("Invalid character in significand", convertToErrorFromString(std::string_view("\0", 1)));
  EXPECT_EQ("Invalid character in significand", convertToErrorFromString(std::string_view("1\0", 2)));
  EXPECT_EQ("Invalid character in significand", convertToErrorFromString(std::string_view("1" "\0" "2", 3)));
  EXPECT_EQ("Invalid character in significand", convertToErrorFromString(std::string_view("1" "\0" "2e1", 5)));
  EXPECT_EQ("Invalid character in exponent", convertToErrorFromString(std::string_view("1e\0", 3)));
  EXPECT_EQ("Invalid character in exponent", convertToErrorFromString(std::string_view("1e1\0", 4)));
  EXPECT_EQ("Invalid character in exponent", convertToErrorFromString(std::string_view("1e1" "\0" "2", 5)));
...
TEST(APFloatTest, StringHexadecimalError) {
  ...
  EXPECT_EQ("Invalid character in significand", convertToErrorFromString(std::string_view("0x\0", 3)));
  EXPECT_EQ("Invalid character in significand", convertToErrorFromString(std::string_view("0x1\0", 4)));
  EXPECT_EQ("Invalid character in significand", convertToErrorFromString(std::string_view("0x1" "\0" "2", 5)));
  EXPECT_EQ("Invalid character in significand", convertToErrorFromString(std::string_view("0x1" "\0" "2p1", 7)));
  EXPECT_EQ("Invalid character in exponent", convertToErrorFromString(std::string_view("0x1p\0", 5)));
  EXPECT_EQ("Invalid character in exponent", convertToErrorFromString(std::string_view("0x1p1\0", 6)));
  EXPECT_EQ("Invalid character in exponent", convertToErrorFromString(std::string_view("0x1p1" "\0" "2", 7)));
...
TEST(DoubleAPFloatTest, isInteger) {
  APFloat F1(-0.0);
  APFloat F2(-0.0);
  bijou::detail::DoubleAPFloat T(APFloat::PPCDoubleDouble(), std::move(F1),
                                std::move(F2));
  EXPECT_TRUE(T.isInteger());
  APFloat F3(3.14159);
  APFloat F4(-0.0);
  bijou::detail::DoubleAPFloat T2(APFloat::PPCDoubleDouble(), std::move(F3),
                                std::move(F4));
  EXPECT_FALSE(T2.isInteger());
  APFloat F5(-0.0);
  APFloat F6(3.14159);
  bijou::detail::DoubleAPFloat T3(APFloat::PPCDoubleDouble(), std::move(F5),
                                std::move(F6));
  ...
  for (auto Tp : Data) {
...

    {
      ...
      EXPECT_EQ(Expected, A1.getCategory())
          << std::format("({0:x} + {1:x}) + ({2:x} + {3:x})", Op1[0], Op1[1],
                         Op2[0], Op2[1]);
    }
    {
      APFloat A1(APFloat::PPCDoubleDouble(), APInt(128, 2, Op1));
      ...
      EXPECT_EQ(Expected, A2.getCategory())
          << std::format("({0:x} + {1:x}) + ({2:x} + {3:x})", Op2[0], Op2[1],
                         Op1[0], Op1[1]);
    }
  }
}
...
TEST(APFloatTest, PPCDoubleDoubleSubtract) {
  ...
  for (auto Tp : Data) {
    ...
    EXPECT_EQ(Expected[0], A1.bitcastToAPInt().getRawData()[0])
        << std::format("({0:x} + {1:x}) - ({2:x} + {3:x})", Op1[0], Op1[1], Op2[0],
                       Op2[1]);
    EXPECT_EQ(Expected[1], A1.bitcastToAPInt().getRawData()[1])
        << std::format("({0:x} + {1:x}) - ({2:x} + {3:x})", Op1[0], Op1[1], Op2[0],
                       Op2[1]);
...
TEST(APFloatTest, PPCDoubleDoubleRemainder) {
  ...
  for (auto Tp : Data) {
    ...
    EXPECT_EQ(Expected[0], A1.bitcastToAPInt().getRawData()[0])
        << std::format("remainder({0:x} + {1:x}), ({2:x} + {3:x}))", Op1[0], Op1[1],
                       Op2[0], Op2[1]);
    EXPECT_EQ(Expected[1], A1.bitcastToAPInt().getRawData()[1])
        << std::format("remainder(({0:x} + {1:x}), ({2:x} + {3:x}))", Op1[0],
                       Op1[1], Op2[0], Op2[1]);
  }
}
...
TEST(APFloatTest, PPCDoubleDoubleMod) {
  ...
  for (auto Tp : Data) {
    ...
    EXPECT_EQ(Expected[0], A1.bitcastToAPInt().getRawData()[0])
        << std::format("fmod(({0:x} + {1:x}),  ({2:x} + {3:x}))", Op1[0], Op1[1],
                       Op2[0], Op2[1]);
    EXPECT_EQ(Expected[1], A1.bitcastToAPInt().getRawData()[1])
        << std::format("fmod(({0:x} + {1:x}), ({2:x} + {3:x}))", Op1[0], Op1[1],
                       Op2[0], Op2[1]);
...
TEST(APFloatTest, PPCDoubleDoubleCompare) {
  ...
  for (auto Tp : Data) {
    ...
    EXPECT_EQ(Expected, A1.compare(A2))
        << std::format("compare(({0:x} + {1:x}), ({2:x} + {3:x}))", Op1[0], Op1[1],
                       Op2[0], Op2[1]);
  }
}
...
TEST(APFloatTest, PPCDoubleDoubleHashValue) {
  ...
  EXPECT_NE(
      hash_fn(APFloat(APFloat::PPCDoubleDouble(), APInt(128, 2, Data1))),
      hash_fn(APFloat(APFloat::PPCDoubleDouble(), APInt(128, 2, Data2))));
}

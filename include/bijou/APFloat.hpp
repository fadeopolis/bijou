// APFloat.hpp - Arbitrary Precision Floating Point
//
// Part of the bijou Project, under the Apache License v2.0 with LLVM Exceptions.
//
// This file is a modified copy of the file "llvm/ADT/APFloat.h" from the LLVM Project.
...
#ifndef BIJOU_ADT_APFLOAT_H
#define BIJOU_ADT_APFLOAT_H
...
#include <type_traits>             // for is_same, enable_if_t, integral_con...
#include "bijou/APInt.hpp"              // for APInt, APInt::APINT_BITS_PER_WORD
#include "bijou/Compiler.hpp"           // for BIJOU_READNONE, BIJOU_READONLY, BIJOU...
#include "bijou/Error.hpp"              // for bijou_unreachable, Expected
#include "bijou/FloatingPointMode.hpp"  // for RoundingMode, RoundingMode::Neares...
...
struct APFloatBase {
  ...
  static const bijou::fltSemantics &EnumToSemantics(Semantics S);
  static Semantics SemanticsToEnum(const bijou::fltSemantics &Sem);

  static const fltSemantics &IEEEhalf() BIJOU_READNONE;
  static const fltSemantics &BFloat() BIJOU_READNONE;
  static const fltSemantics &IEEEsingle() BIJOU_READNONE;
  static const fltSemantics &IEEEdouble() BIJOU_READNONE;
  static const fltSemantics &IEEEquad() BIJOU_READNONE;
  static const fltSemantics &PPCDoubleDouble() BIJOU_READNONE;
  static const fltSemantics &x87DoubleExtended() BIJOU_READNONE;

  /// A Pseudo fltsemantic used to construct APFloats that cannot conflict with
  /// anything real.
  static const fltSemantics &Bogus() BIJOU_READNONE;
  ...
  using roundingMode = bijou::RoundingMode;
...
public:
  ...
  opStatus convertToInteger(std::span<integerPart>, unsigned int, bool,
  ...
  /// @name IEEE-754R 5.7.2 General operations.
  /// @{
...
private:
  ...
  /// @name Arithmetic on special values.
  /// @{
  ...
  opStatus convertToSignExtendedInteger(std::span<integerPart>,
...
class APFloat : public APFloatBase {
  ...
  APFloat() : U(IEEEdouble()) {
    bijou_unreachable("This is a workaround for old clang.");
  }
...
public:
  ...
  BIJOU_DUMP_METHOD void dump() const;

  bool getExactInverse(APFloat *inv) const {
    APFLOAT_DISPATCH_ON_SEMANTICS(getExactInverse(inv));
  }

  friend std::hash<APFloat>;
...
template <> struct hash<bijou::detail::IEEEFloat> {
  std::size_t operator()(const bijou::detail::IEEEFloat &Arg) const;
};

template <> struct hash<bijou::detail::DoubleAPFloat> {
  std::size_t operator()(const bijou::detail::DoubleAPFloat &Arg) const;
};

template <> struct hash<bijou::APFloat> {
  std::size_t operator()(const bijou::APFloat &Arg) const;
};

} // namespace std

#undef APFLOAT_DISPATCH_ON_SEMANTICS
#endif // BIJOU_ADT_APFLOAT_HPP

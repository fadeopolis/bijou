...
//
// This file is a modified copy of the file "lib/Support/APFloat.cpp" from the LLVM Project.
...
///
/// @file
/// @brief// This file implements a class to represent arbitrary precision floating
/// point values and provide a variety of arithmetic operations on them.
///

#include "bijou/APFloat.hpp"
#include <algorithm>            // for equal, max, min
#include <ctype.h>              // for tolower
#include <climits>              // for INT_MAX, INT_MIN
#include <cstdio>               // for fprintf, stderr, FILE
#include <cstdint>              // for uint64_t, uint32_t, uint8_t
#include <cstring>              // for memset, size_t, memcpy
#include <optional>             // for optional
#include <span>                 // for span
#include <string>               // for basic_string, char_traits
#include <vector>               // for vector
#include "bijou/APSInt.hpp"          // for APSInt
#include "bijou/Error.hpp"           // for Expected, bijou_unreachable, Error
#include "bijou/Hashing.hpp"         // for hash_combine, hash_combine_range
#include "bijou/MathExtras.hpp"      // for NextPowerOf2

#ifdef AP_USE_IOSTREAM
#  include <iostream>
#endif
...
inline unsigned GetAutoSenseRadix(std::string_view &Str) {
  if (Str.empty())
    return 10;
...
bool getAsInteger(std::string_view Str, unsigned Radix, APInt &Result) {
...
#define APFLOAT_DISPATCH_ON_SEMANTICS(METHOD_CALL)                             \
  do {                                                                         \
    ...
    bijou_unreachable("Unexpected semantics");                                 \
  } while (false)
...
namespace bijou {
  ...
  const bijou::fltSemantics &APFloatBase::EnumToSemantics(Semantics S) {
    switch (S) {
    case S_IEEEhalf:
      return IEEEhalf();
    case S_BFloat:
    ...
    bijou_unreachable("Unrecognised floating semantics");
  }

  APFloatBase::Semantics
  APFloatBase::SemanticsToEnum(const bijou::fltSemantics &Sem) {
    if (&Sem == &bijou::APFloat::IEEEhalf())
      return S_IEEEhalf;
    else if (&Sem == &bijou::APFloat::BFloat())
      return S_BFloat;
    else if (&Sem == &bijou::APFloat::IEEEsingle())
      return S_IEEEsingle;
    else if (&Sem == &bijou::APFloat::IEEEdouble())
      return S_IEEEdouble;
    else if (&Sem == &bijou::APFloat::x87DoubleExtended())
      return S_x87DoubleExtended;
    else if (&Sem == &bijou::APFloat::IEEEquad())
      return S_IEEEquad;
    else if (&Sem == &bijou::APFloat::PPCDoubleDouble())
      return S_PPCDoubleDouble;
    else
      bijou_unreachable("Unknown floating semantics");
  }
...
static inline Error createError(std::string_view Err) {
  return Error(Err);
}
...
/* Return the value of a decimal exponent of the form
   ...
   appropriate sign.  */
static Expected<int> readExponent(std::string_view::iterator begin,
                                  std::string_view::iterator end) {
  bool isNegative;
  unsigned int absExponent;
  const unsigned int overlargeExponent = 24000;  /* FIXME.  */
  std::string_view::iterator p = begin;
...
static Expected<int> totalExponent(std::string_view::iterator p,
                                   std::string_view::iterator end,
...
static Expected<std::string_view::iterator>
skipLeadingZeroesAndAnyDot(std::string_view::iterator begin, std::string_view::iterator end,
                           std::string_view::iterator *dot) {
  std::string_view::iterator p = begin;
  *dot = end;
  while (p != end && *p == '0')
    p++;
...
static Error interpretDecimal(std::string_view::iterator begin,
                              std::string_view::iterator end, decimalInfo *D) {
  std::string_view::iterator dot = end;

  auto PtrOrErr = skipLeadingZeroesAndAnyDot(begin, end, &dot);
  if (!PtrOrErr)
    return PtrOrErr.takeError();
  std::string_view::iterator p = *PtrOrErr;
...
trailingHexadecimalFraction(std::string_view::iterator p, std::string_view::iterator end,
...
bool IEEEFloat::roundAwayFromZero(roundingMode rounding_mode,
  ...
  bijou_unreachable("Invalid rounding mode found");
}
...
IEEEFloat::convertFromZeroExtendedInteger(const integerPart *parts,
  ...
  APInt api = APInt(width, std::span(parts, partCount));
...
IEEEFloat::convertFromHexadecimalString(std::string_view s,
                                        roundingMode rounding_mode) {
  ...
  std::string_view::iterator begin = s.begin();
  ...
  if (!PtrOrErr)
    return PtrOrErr.takeError();
  std::string_view::iterator p = *PtrOrErr;
  std::string_view::iterator firstSignificantDigit = p;
...
Expected<IEEEFloat::opStatus>
IEEEFloat::convertFromDecimalString(std::string_view str, roundingMode rounding_mode) {
  decimalInfo D;
  opStatus fs;
...
bool IEEEFloat::convertFromStringSpecials(std::string_view str) {
  ...
  if (str.starts_with("nan") || str.starts_with("NaN")) {
    ...
    APInt Payload;
    if (!getAsInteger(str, Radix, Payload)) {
...
Expected<IEEEFloat::opStatus>
IEEEFloat::convertFromString(std::string_view str, roundingMode rounding_mode) {
  if (str.empty())
  ...
  if (slen >= 2 && p[0] == '0' && (p[1] == 'x' || p[1] == 'X')) {
    ...
    return convertFromHexadecimalString(std::string_view(p + 2, slen - 2),
                                        rounding_mode);
  }

  return convertFromDecimalString(std::string_view(p, slen), rounding_mode);
}
...
APInt IEEEFloat::convertF80LongDoubleAPFloatToAPInt() const {
  assert(semantics == (const bijou::fltSemantics*)&semX87DoubleExtended);
  assert(partCount()==2);
...
APInt IEEEFloat::convertPPCDoubleDoubleAPFloatToAPInt() const {
  assert(semantics == (const bijou::fltSemantics *)&semPPCDoubleDoubleLegacy);
  assert(partCount()==2);
...
APInt IEEEFloat::convertQuadrupleAPFloatToAPInt() const {
  assert(semantics == (const bijou::fltSemantics*)&semIEEEquad);
  assert(partCount()==2);
...
APInt IEEEFloat::convertDoubleAPFloatToAPInt() const {
  assert(semantics == (const bijou::fltSemantics*)&semIEEEdouble);
  assert(partCount()==1);
...
APInt IEEEFloat::convertFloatAPFloatToAPInt() const {
  assert(semantics == (const bijou::fltSemantics*)&semIEEEsingle);
  assert(partCount()==1);
...
APInt IEEEFloat::convertBFloatAPFloatToAPInt() const {
  assert(semantics == (const bijou::fltSemantics *)&semBFloat);
  assert(partCount() == 1);
...
APInt IEEEFloat::convertHalfAPFloatToAPInt() const {
  assert(semantics == (const bijou::fltSemantics*)&semIEEEhalf);
  assert(partCount()==1);
...
APInt IEEEFloat::bitcastToAPInt() const {
  if (semantics == (const bijou::fltSemantics*)&semIEEEhalf)
    return convertHalfAPFloatToAPInt();

  if (semantics == (const bijou::fltSemantics *)&semBFloat)
    return convertBFloatAPFloatToAPInt();

  if (semantics == (const bijou::fltSemantics*)&semIEEEsingle)
    return convertFloatAPFloatToAPInt();

  if (semantics == (const bijou::fltSemantics*)&semIEEEdouble)
    return convertDoubleAPFloatToAPInt();

  if (semantics == (const bijou::fltSemantics*)&semIEEEquad)
    return convertQuadrupleAPFloatToAPInt();

  if (semantics == (const bijou::fltSemantics *)&semPPCDoubleDoubleLegacy)
...
namespace {
  void append(std::string &Buffer, std::string_view Str) {
  ...
  void AdjustToPrecision(std::string &buffer,
                         int &exp, unsigned FormatPrecision) {
...
void IEEEFloat::toString(std::string &Str, unsigned FormatPrecision,
...
Expected<APFloat::opStatus> DoubleAPFloat::convertFromString(std::string_view S,
                                                             roundingMode RM) {
  assert(Semantics == &semPPCDoubleDouble && "Unexpected Semantics");
...
Expected<APFloat::opStatus> APFloat::convertFromString(std::string_view Str,
                                                       roundingMode RM) {
  APFLOAT_DISPATCH_ON_SEMANTICS(convertFromString(Str, RM));
}

APFloat::APFloat(const fltSemantics &Semantics, std::string_view S)
    : APFloat(Semantics) {
...
#ifdef AP_USE_IOSTREAM
void APFloat::print(std::iostream &OS) const {
  std::string Buffer;
  toString(Buffer);
  OS << Buffer << "\n";
}
...
void APFloat::print(FILE *O) const {
  std::string Buffer;
  toString(Buffer);
  fprintf(O, "%s\n", Buffer.c_str());
}

BIJOU_DUMP_METHOD void APFloat::dump() const {
#ifdef AP_USE_IOSTREAM
  print(std::cerr);
#else
  print(stderr);
...
APFloat::opStatus APFloat::convertToInteger(APSInt &result,
  ...
  std::vector<uint64_t> parts(result.getNumWords());
...
double APFloat::convertToDouble() const {
  if (&getSemantics() == (const bijou::fltSemantics *)&semIEEEdouble)
...
float APFloat::convertToFloat() const {
  if (&getSemantics() == (const bijou::fltSemantics *)&semIEEEsingle)
...
std::size_t std::hash<bijou::detail::IEEEFloat>::operator()(const bijou::detail::IEEEFloat &Arg) const {
...
std::size_t std::hash<bijou::detail::DoubleAPFloat>::operator()(const bijou::detail::DoubleAPFloat &Arg) const {
  if (Arg.Floats) {
    std::hash<APFloat> hash_fn;
    return hash_combine(hash_fn(Arg.Floats[0]), hash_fn(Arg.Floats[1]));
  }
  return hash_combine(Arg.Semantics);
}

std::size_t std::hash<bijou::APFloat>::operator()(const bijou::APFloat &Arg) const {
  if (APFloat::usesLayout<detail::IEEEFloat>(Arg.getSemantics())) {
    std::hash<bijou::detail::IEEEFloat> hash_fn;
    return hash_fn(Arg.U.IEEE);
  }
  if (APFloat::usesLayout<detail::DoubleAPFloat>(Arg.getSemantics())) {
    std::hash<bijou::detail::DoubleAPFloat> hash_fn;
    return hash_fn(Arg.U.Double);
  }
  bijou_unreachable("Unexpected semantics");
}

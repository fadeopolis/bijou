...
//
// This file is a modified copy of the file "lib/Support/APInt.cpp" from the LLVM Project.
...
#include <algorithm>                // for std::equal, std::min,  std::reverse
#include <bit>                      // std::for bit_cast, std::endian
...
#include "bijou/Error.hpp"          // for bijou_unreachable
#include "bijou/Hashing.hpp"        // for hash_combine, hash_combine_range
#include "bijou/MathExtras.hpp"     // for Lo_32, SignExtend64, Hi_32, Make_64
#include "bijou/SwapByteOrder.hpp"  // for ByteSwap_64, ByteSwap_16, ByteSwa...

using namespace bijou;

#define BIJOU_DEBUG(EXPR) static_cast<void>(0)
...
void APInt::initFromArray(std::span<const uint64_t> bigVal) {
...
APInt::APInt(unsigned numBits, std::span<const uint64_t> bigVal) : BitWidth(numBits) {
  initFromArray(bigVal);
}

APInt::APInt(unsigned numBits, unsigned numWords, const uint64_t bigVal[])
    : BitWidth(numBits) {
  initFromArray(std::span(bigVal, numWords));
}
...
APInt APInt::extractBits(unsigned numBits, unsigned bitPosition) const {
  ...
  if (loBit == 0)
    return APInt(numBits, std::span(U.pVal + loWord, 1 + hiWord - loWord));
...
std::size_t std::hash<APInt>::operator()(const APInt &Arg) const {
  if (Arg.isSingleWord())
...
unsigned APInt::countLeadingZerosSlowCase() const {
  ...
  for (int i = getNumWords()-1; i >= 0; --i) {
    ...
    if (V == 0)
      ...
      Count += bijou::countLeadingZeros(V);
      break;
    }
  }
...
unsigned APInt::countLeadingOnesSlowCase() const {
  ...
  int i = getNumWords() - 1;
  unsigned Count = bijou::countLeadingOnes(U.pVal[i] << shift);
  if (Count == highWordBits) {
    for (i--; i >= 0; --i) {
      if (U.pVal[i] == WORDTYPE_MAX)
        Count += APINT_BITS_PER_WORD;
      else {
        Count += bijou::countLeadingOnes(U.pVal[i]);
        break;
      }
    }
  }
...
unsigned APInt::countTrailingZerosSlowCase() const {
  ...
  for (; i < getNumWords() && U.pVal[i] == 0; ++i)
    Count += APINT_BITS_PER_WORD;
  if (i < getNumWords())
    Count += bijou::countTrailingZeros(U.pVal[i]);
...
unsigned APInt::countTrailingOnesSlowCase() const {
  ...
  for (; i < getNumWords() && U.pVal[i] == WORDTYPE_MAX; ++i)
    Count += APINT_BITS_PER_WORD;
  if (i < getNumWords())
    Count += bijou::countTrailingOnes(U.pVal[i]);
...
unsigned APInt::countPopulationSlowCase() const {
  ...
  for (unsigned i = 0; i < getNumWords(); ++i)
    Count += bijou::countPopulation(U.pVal[i]);
  return Count;
}
...
APInt APInt::reverseBits() const {
  switch (BitWidth) {
  case 64:
    return APInt(BitWidth, bijou::reverseBits<uint64_t>(U.VAL));
  case 32:
    return APInt(BitWidth, bijou::reverseBits<uint32_t>(U.VAL));
  case 16:
    return APInt(BitWidth, bijou::reverseBits<uint16_t>(U.VAL));
  case 8:
    return APInt(BitWidth, bijou::reverseBits<uint8_t>(U.VAL));
  case 0:
    return *this;
  default:
    break;
...
APInt bijou::APIntOps::GreatestCommonDivisor(APInt A, APInt B) {
...
APInt bijou::APIntOps::RoundDoubleToAPInt(double Double, unsigned width) {
...
#ifdef KNUTH_DEBUG
#define DEBUG_KNUTH(X) BIJOU_DEBUG(X)
#else
...
void APInt::toString(std::string &Str, unsigned Radix,
  ...
  if (formatAsCLiteral) {
    switch (Radix) {
      ...
      case 16:
        ...
        break;
      default:
        bijou_unreachable("Invalid radix!");
    }
  }
...
BIJOU_DUMP_METHOD void APInt::dump() const {
  std::string S, U;
  this->toStringUnsigned(U);
  this->toStringSigned(S);
#ifdef AP_USE_IOSTREAM
  std::cerr << "APInt(" << BitWidth << "b, " << U << "u " << S << "s)\n";
#else
  fprintf(stderr, "APInt(%db, %su, %ss)\n", BitWidth, U.c_str(), S.c_str());
#endif
}

#ifdef AP_USE_IOSTREAM
void APInt::print(std::iostream &OS, bool isSigned) const {
  std::string S;
...
void APInt::print(FILE *O, bool isSigned) const {
  std::string S;
  this->toString(S, 10, isSigned, /* formatAsCLiteral = */false);
  fprintf(O, "%s", S.c_str());
}
...
APInt bijou::APIntOps::RoundingUDiv(const APInt &A, const APInt &B,
                                   APInt::Rounding RM) {
  ...
  bijou_unreachable("Unknown APInt::Rounding enum");
}

APInt bijou::APIntOps::RoundingSDiv(const APInt &A, const APInt &B,
                                   APInt::Rounding RM) {
  switch (RM) {
  case APInt::Rounding::DOWN:
  case APInt::Rounding::UP: {
...
std::optional<APInt>
bijou::APIntOps::SolveQuadraticEquationWrap(APInt A, APInt B, APInt C,
                                           unsigned RangeWidth) {
  ...
  BIJOU_DEBUG(dbgs() << __func__ << ": solving " << A << "x^2 + " << B
                     << "x + " << C << ", rw:" << RangeWidth << '\n');

  // Identify 0 as a (non)solution immediately.
  if (C.sextOrTrunc(RangeWidth).isZero()) {
    BIJOU_DEBUG(dbgs() << __func__ << ": zero solution\n");
  ...
  BIJOU_DEBUG(dbgs() << __func__ << ": updated coefficients " << A << "x^2 + "
                     << B << "x + " << C << ", rw:" << RangeWidth << '\n');
  ...
  if (!InexactSQ && Rem.isZero()) {
    BIJOU_DEBUG(dbgs() << __func__ << ": solution (root): " << X << '\n');
    return X;
  }
  ...
  if (!SignChange) {
    BIJOU_DEBUG(dbgs() << __func__ << ": no valid solution\n");
    return std::nullopt;
  }

  X += 1;
  BIJOU_DEBUG(dbgs() << __func__ << ": solution (wrap): " << X << '\n');
  return X;
}

std::optional<unsigned>
bijou::APIntOps::GetMostSignificantDifferentBit(const APInt &A, const APInt &B) {
...
APInt bijou::APIntOps::ScaleBitMask(const APInt &A, unsigned NewBitWidth) {
...
void bijou::StoreIntToMemory(const APInt &IntVal, uint8_t *Dst,
                            unsigned StoreBytes) {
...
void bijou::LoadIntFromMemory(APInt &IntVal, const uint8_t *Src,
                             unsigned LoadBytes) {
